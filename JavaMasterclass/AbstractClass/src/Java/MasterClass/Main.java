...
1. ListItem (abstract class)
    ...
    - A constructor that takes an Object and initialises the field value with the parameter
        that was passed in.
    - And seven methods:
...
2. Node (concrete class)
    ...
    - And five methods which are package-private:
        ...
        - compareTo(), takes a ListItem and compares it to the ListItem that called this method.
            Use value from ListItem for comparison. If this value is greater than the value
            that was passed in, then it should return a number greater than zero.
...
5. SearchTree (concrete class)
    ...
    - And five methods:
        ...
        - traverse(), takes the root as an argument and does not return anything.
            It usListItem item1 = new Node(1);es recursion to visit all the branches in the tree (Inorder).
...
TIP:  Inorder = print the previous node, then the parent node,
    and then the next node (left -> node -> right).
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        ListItem item6 = new Node("6");
        ListItem item7 = new Node("7");

        NodeList myList = new SearchTree(item4);
